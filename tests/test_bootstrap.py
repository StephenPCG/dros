...
def test_bootstrap_disables_cloud_init_network_management_when_cloud_init_exists(
    tmp_path: Path,
) -> None:
    settings = _settings(tmp_path)
    settings.paths.configs.mkdir(parents=True)
    ...
    for relative in [
        ...
        path = settings.sys_root / relative
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("# cloud-init network\n", encoding="utf-8")
    ...
    drop_in = settings.sys_root / "etc/cloud/cloud.cfg.d/99-dros-network.cfg"
    assert drop_in.read_text(encoding="utf-8") == (
    ...
    for relative in [
        ...
        assert not (settings.sys_root / relative).exists()
    commands = [" ".join(action.command or []) for action in result.actions]
    assert any("systemctl disable --now wide-dhcpv6-client.service" in command for command in commands)


def test_bootstrap_can_preserve_cloud_init_network_and_wide_dhcpv6_service(
    tmp_path: Path,
) -> None:
    settings = _settings(tmp_path)
    settings.paths.configs.mkdir(parents=True)
...
metadata:
  name: system
spec:
  disableCloudInitNetwork: false
  wideDhcpv6ClientService: true
""".lstrip(),
        encoding="utf-8",
    )
    cloud_dir = settings.sys_root / "etc/cloud"
    cloud_dir.mkdir(parents=True)
    (cloud_dir / "cloud.cfg").write_text("# cloud-init\n", encoding="utf-8")
    eni = settings.sys_root / "etc/network/interfaces.d/50-cloud-init.cfg"
    eni.parent.mkdir(parents=True, exist_ok=True)
    eni.write_text("# keep cloud-init network\n", encoding="utf-8")
    ...
    assert not (settings.sys_root / "etc/cloud/cloud.cfg.d/99-dros-network.cfg").exists()
    assert eni.exists()
    commands = [" ".join(action.command or []) for action in result.actions]
    assert not any("wide-dhcpv6-client.service" in command for command in commands)
