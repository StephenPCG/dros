...
MANAGED_FILES = frozenset(
    {
        ...
        "/etc/systemd/system/dros-tailscaled-*.service",
        "/etc/wireguard/*.conf",
...
WGSD_CLIENT_BINARY = "/usr/local/bin/wgsd-client"
TAILSCALE_SOCKET_DIR = "tailscale"
...
def update(context: UpdateContext, objects: list[ConfigObject]) -> None:
    ...
    for obj in ordered_interfaces:
        ...
        if iface_type == "tailscale":
            update_tailscale_interface(context, obj.name, config, devgroups)
            continue
...
def update_tailscale_interface(
    context: UpdateContext,
    name: str,
    config: InterfaceConfig,
    devgroups: dict[str, int],
) -> None:
    context.executor.ensure_dir(tailscale_socket_path(context.settings, name).parent)
    context.executor.ensure_dir(_tailscale_state_dir(name, config))
    service_name = tailscale_service_name(name)
    service_changed = context.executor.write_file(
        f"/etc/systemd/system/{service_name}",
        _render_tailscaled_service(context, name, config),
    )
    if service_changed:
    ...
    context.executor.run(_tailscale_up_command(context.settings, name, config), real_only=True)
    apply_runtime_interface_properties(context, name, config, devgroups)
...
def tailscale_service_name(name: str) -> str:
    return f"dros-tailscaled-{_safe_name(name)}.service"


def tailscale_socket_path(settings: DrosSettings, name: str) -> Path:
    return settings.paths.run / TAILSCALE_SOCKET_DIR / f"{_safe_name(name)}.sock"


def _tailscale_state_dir(name: str, config: InterfaceConfig) -> str:
    if config.state_dir is not None:
        return config.state_dir
    return f"/var/lib/tailscale/{_safe_name(name)}"


def _render_tailscaled_service(
    context: UpdateContext,
    name: str,
    config: InterfaceConfig,
) -> str:
    socket_path = str(tailscale_socket_path(context.settings, name))
    state_dir = _tailscale_state_dir(name, config)
    args = [
        "/usr/sbin/tailscaled",
        f"--tun={name}",
    ...
    if config.no_logs_no_support:
        args.append("--no-logs-no-support")
    args.extend(config.extra_daemon_args)
    return "\n".join(
        [
            "# Generated by DROS. Manual changes will be overwritten.",
            "[Unit]",
            f"Description=DROS Tailscale daemon for {name}",
            "After=network-online.target",
            "Wants=network-online.target",
            "",
            "[Service]",
            "Type=notify",
            f"ExecStartPre={_shell_command(['/usr/bin/install', '-d', '-m', '0755', str(Path(socket_path).parent), state_dir])}",
            f"ExecStart={_shell_command(args)}",
            "Restart=on-failure",
            "",
            "[Install]",
...
def _tailscale_up_command(
    ...
    command = [
        "tailscale",
        f"--socket={tailscale_socket_path(settings, name)}",
        "up",
        "--reset",
    ]
    if config.login_server:
        command.append(f"--login-server={config.login_server}")
    if config.hostname:
        command.append(f"--hostname={config.hostname}")
    command.extend(
        [
            f"--accept-routes={_bool_arg(config.accept_routes)}",
            f"--accept-dns={_bool_arg(config.accept_dns)}",
            f"--netfilter-mode={config.netfilter_mode}",
            f"--timeout={config.up_timeout}",
        ]
    )
    if config.advertise_routes:
        command.append(f"--advertise-routes={','.join(config.advertise_routes)}")
    if config.advertise_tags:
        command.append(f"--advertise-tags={','.join(config.advertise_tags)}")
    if config.snat_subnet_routes is not None:
        command.append(f"--snat-subnet-routes={_bool_arg(config.snat_subnet_routes)}")
    if config.stateful_filtering is not None:
        command.append(f"--stateful-filtering={_bool_arg(config.stateful_filtering)}")
    if config.ssh:
        command.append("--ssh=true")
    if config.shields_up:
        command.append("--shields-up=true")
    if config.operator:
        command.append(f"--operator={config.operator}")
    command.extend(config.extra_up_args)
    return command
...
def _validate_interface(
    ...
    if iface_type == "vlan":
        ...
        _validate_tailscale(obj, config, errors)
    return errors
...
def _validate_tailscale(obj: ConfigObject, config: InterfaceConfig, errors: list[str]) -> None:
    for label, value in (
        ("spec.loginServer", config.login_server),
        ("spec.hostname", config.hostname),
        ("spec.operator", config.operator),
        ("spec.stateDir", config.state_dir),
        ("spec.upTimeout", config.up_timeout),
    ):
        if value is not None:
            _validate_single_line(obj, label, value, errors)
    for field_name, values in (
        ("advertiseRoutes", config.advertise_routes),
        ("advertiseTags", config.advertise_tags),
        ("extraDaemonArgs", config.extra_daemon_args),
        ("extraUpArgs", config.extra_up_args),
    ):
        for index, value in enumerate(values):
            _validate_single_line(obj, f"spec.{field_name}[{index}]", value, errors)
    for index, route in enumerate(config.advertise_routes):
        try:
            ipaddress.ip_network(route, strict=False)
        except ValueError as exc:
            errors.append(
                f"Interface/{obj.name}: spec.advertiseRoutes[{index}] is not a valid CIDR: {exc}"
            )
    for index, tag in enumerate(config.advertise_tags):
        if not tag.startswith("tag:"):
            errors.append(f"Interface/{obj.name}: spec.advertiseTags[{index}] must start with tag:")
    if config.port is not None and not 0 <= config.port <= 65535:
        errors.append(f"Interface/{obj.name}: spec.port must be between 0 and 65535")
...
def _validate_single_line(
    ...
    if any(char in str(value) for char in "\r\n"):
        errors.append(f"{obj.kind}/{obj.name}: {label} must be single-line")
...
def _write_wgsd_cron(context: UpdateContext, name: str, config: InterfaceConfig) -> bool:
    ...
    if not context.executor.exists(WGSD_CLIENT_BINARY):
        _warn(
            context,
            f"Interface/{name}: {WGSD_CLIENT_BINARY} is not installed; "
            "skip wgsdClient cron; run gw install wgsd-client",
        )
        return context.executor.delete_file(path)
    lines = [
        "# Generated by DROS. Manual changes will be overwritten.",
